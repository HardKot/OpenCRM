 ...
 *
 * <p>ROLE_ADMIN — passes all permission checks unconditionally. ROLE_OWNER — passes all regular
 * permission checks (permissions are ignored), but still cannot access ROLE_ADMIN-only endpoints.
 * ROLE_EMPLOYEE — access is gated by the explicit UserPermission set stored on the user.
 */
@Component

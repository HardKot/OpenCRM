...
public interface IUserRepository extends JpaRepository<User, UUID> {
    ...
    Optional<User> findByEmployeeId(@Param("employeeId") Long employeeId, @Param("tenantId") UUID tenantId);

    @Query("SELECT CASE WHEN COUNT(u) > 0 THEN true ELSE false END FROM User u WHERE u.email = :email AND u.tenant.id = :tenantId AND u.entityName = 'EMPLOYEE'")

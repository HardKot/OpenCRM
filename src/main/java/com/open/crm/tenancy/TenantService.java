...
public class TenantService {
    ...
    public Tenant createTenant(String email) throws TenantException {
        if (Objects.isNull(email) || email.isBlank())
            throw new TenantException("Email cannot be empty");
        if (email.length() > 255)
            throw new TenantException("Email cannot be longer than 255 characters");
        if (!EMAIL_REGEX.matcher(email).matches())
            throw new TenantException("Email is not valid");
        ...
        ApplicationEmailEvent emailEvent = new ApplicationEmailEvent(this, user.getEmail(), "Welcome to OpenCRM",
                "email/welcome-email", Map.of("email", user.getEmail(), "password", password));

        ApplicationSchemaEvent schemaEvent = new ApplicationSchemaEvent(this, tenant.getSchemaName(), tenant.getId(),
                user);

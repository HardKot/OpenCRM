    ...
    builder
      .addMatcher(
        ...
        (state, action) => {
          ...
          if (isUserRole(action.payload.role))
            authData.role = action.payload.role;
          for (const permission of action.payload.permissions) {
            if (isUserPermission(permission))
              authData.permissions.push(permission);
          }
      ...
      .addMatcher(authApi.endpoints.holdSession.matchRejected, (state) => {
        state.authData = null;
        state.isAuth = false;
        state.entity = null;
      });

...
const ModeLogin = ({ onModeChange }: ModeComponentProps) => {
    ...
    return (
        <ModeWrapper>
            <Button onClick={() => onModeChange(AuthPageMode.FORGOT_PASSWORD)}>{t('auth.forgotPassword')}</Button>
            <Button onClick={() => onModeChange(AuthPageMode.CREATE_TENANT)}>{t('auth.createTenant')}</Button>
        </ModeWrapper>
    );
}
...
const ModeForgotPassword = ({ onModeChange }: ModeComponentProps) => {
    ...
    return (
        <ModeWrapper>
            <Button onClick={() => onModeChange(AuthPageMode.LOGIN)}>{t('auth.backToLogin')}</Button>
            <Button onClick={() => onModeChange(AuthPageMode.CREATE_TENANT)}>{t('auth.createTenant')}</Button>
        </ModeWrapper>
    );
}
